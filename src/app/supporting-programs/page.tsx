...
import { Card, CardBody, CardHeader, Button, Spinner, Image, Pagination, Divider } from "@heroui/react";
...
export default function SupportingPrograms() {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (accessToken && userId) {
                    ...
                    const registeredIds = registeredData.data.map((p: SupportingProgram) => p.id);
    ...
    const handleRegister = async (programId: number) => {
        ...
        const updatedPrograms = allPrograms.filter((p) => !([...registeredProgramIds, programId].includes(p.id)));
        setAllPrograms(updatedPrograms);
        setTotalPages(Math.ceil(updatedPrograms.length / pageSize));

        const currentPagePrograms = updatedPrograms.slice((pageIndex - 1) * pageSize, pageIndex * pageSize);
        if (currentPagePrograms.length === 0 && pageIndex > 1) {
            setPageIndex((prev) => prev - 1);
        }
    };

    const currentPrograms = allPrograms.slice((pageIndex - 1) * pageSize, pageIndex * pageSize);
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="py-16 px-10 mb-10 rounded-xl max-w-7xl mx-auto flex gap-12 items-center">
                <div className="w-1/2">
                    ...
                    <MotionHeading className="mt-4 text-lg font-noto-sans font-normal">
                        Tìm hiểu các chương trình hỗ trợ tâm lý được thiết kế để giúp bạn cải thiện sức khỏe tinh thần
                        và vượt qua thử thách trong cuộc sống. Chúng tôi mang đến các giải pháp phù hợp để bạn có thể
                        tiếp cận sự hỗ trợ chuyên nghiệp một cách dễ dàng.
                    </MotionHeading>
                </div>
            ...
            {loading ? (
                ...
                <>
                    {localStorage.getItem("accessToken") && allPrograms.length === 0 && registeredProgramIds.length > 0 ? (
