...
class WebRTCService {
    ...
    private onRemoteStreamCallback: ((stream: MediaStream, connectionId: string) => void) | null = null;
    ...
    constructor() {
        ...
        this.connection = new signalR.HubConnectionBuilder()
            .withUrl("http://localhost:7096/hub/webrtc")
    ...
    private setupSignalRCallbacks() {
        ...
        this.connection.on('UserJoined', async (connectionId: string, username: string) => {
        ...
        this.connection.on('ReceiveOffer', async (offer: string, fromConnectionId: string) => {
            console.log(`Received offer from: ${fromConnectionId}`);
            const peerConnection = await this.createPeerConnection(fromConnectionId);
        ...
        this.connection.on('ReceiveAnswer', async (answer: string, fromConnectionId: string) => {
        ...
        this.connection.on('ReceiveIceCandidate', async (candidate: string, fromConnectionId: string) => {
     ...
     */
    private async createPeerConnection(connectionId: string): Promise<RTCPeerConnection> {
